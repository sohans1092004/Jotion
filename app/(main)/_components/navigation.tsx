...
import { ChevronsLeft, Icon, MenuIcon, Plus, PlusCircle, Search, Settings, Trash } from "lucide-react";
import { useParams, usePathname, useRouter } from "next/navigation";
import { useRef,ElementRef,useState, use, useEffect } from "react";
...
export const Navigation = () => {
        ...
        if(sidebarRef.current && navbarRef.current)
            {
                ...
                navbarRef.current.style.setProperty(
                    "left", isMobile ? "100%" : "360px"
                );
                navbarRef.current.style.setProperty(
                    "width",isMobile ? "0": "calc(100% -360px)"
                );
    ...
    return (
        ...
        <div 
            ref={navbarRef}
            className={cn(
                "absolute top-0 z-[99999] left-60 w-[calc(100%-360px)]", isResetting && "transition-all ease-in-out duration-300", isMobile && "left-0 w-full"
            )}
        >
            {!!params.documentId ? (
